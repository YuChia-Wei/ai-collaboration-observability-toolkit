...
3. Phoenix is optional and, in Evaluation mode, receives already-redacted traces by default. The `x-ai-observability-phoenix: false` OTLP header or legacy boolean resource attribute `ai_context.export.phoenix=false` opts a trace out. Core and Corporate expose no Phoenix route.
